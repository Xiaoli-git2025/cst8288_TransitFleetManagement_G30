...
<%
    ...
    StationTimeDTO cur_time = (StationTimeDTO) request.getAttribute("logtime");
%>
<!DOCTYPE html>
<html>
    <head>
        ...
        <title>Transit Fleet Management System - Edit Stop Time</title>
    </head>
    <body class="body">
        <!-- Header -->
        <div class="header">
            <h1>Operator - Edit Stop Time</h1>
            ...
            <div class="container">
                <div class="box_with_menu">
                    <form action="${pageContext.request.contextPath}/LogStopTime" method="post">
                        <label for="time_id">Time ID:</label>
                        <input type="text" id="time_id" name="time_id" readonly value="<%= cur_time.getTimeId() %>">
                        ...
                        <input type="time" id="ra_time" name="ra_time"
                            value="<%= cur_time.getArriveTime().toString().substring(0,5) %>">
                        <label for="rd_time">Real Depart Time:</label>
                        <input type="time" id="rd_time" name="rd_time"
                               value="<%= cur_time.getDepartTime().toString().substring(0,5) %>">
                        <label for="note">Note:</label>
                        <textarea id="note" name="note" rows="4" cols="50"><%= cur_time.getNote() %></textarea>
                        <label for="action"></label>
                        <input type="submit" name="action" class="submit-btn" value="Update Stop Time">
                    </form>
                </div>
            </div>
        </div>
