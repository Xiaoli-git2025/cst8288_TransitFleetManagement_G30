...
<%
    ...
    int alert_type_id = cur_alert.getAlertId();
    int component_id = cur_alert.getComponentId();
%>
<!DOCTYPE html>
<html>
    <head>
                    ...
                    <form action="${pageContext.request.contextPath}/MaintenanceAlert" method="post">
                        ...
                        <select name="component_id">
                            <% for (VehicleComponentDTO component : components) {
                                ...
                                String selected = (id == component_id) ? "selected" : "";
                            %>
                                <option value="<%= id %>" <%= selected %>><%= component.getComponentName() %></option>
                            <% } %>
                        </select>
                        <label for="alert_type">Alert Type</label>
                        <select name="alert_type">
                            <% for (AlertDTO alert_type : alert_types) {
                                int id = alert_type.getAlertId();
                                String selected = (id == alert_type_id) ? "selected" : "";
                            %>
                                <option value="<%= id %>" <%= selected %>><%= alert_type.getAlertType() %></option>
                            <% } %>
                        </select>
                        <label for="selectedDate">Alert Date:</label>
                        <input type="date" id="selectedDate" name="alert_date" value="<%= cur_alert.getAlertDate() %>">
                        
                        <label for="action"></label>
                        <input type="submit" name="action" class="submit-btn" value="Update Alert">
                    </form>
                </div>
            </div>
        </div>
